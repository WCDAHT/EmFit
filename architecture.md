...
```rust
/// One filesystem reader. Registered explicitly (STANDARDS sec 4.3).
pub trait FsScanner: Send + Sync {
    fn name(&self) -> &str;                       // "NTFS", "ext4", "Directory walk"
    fn caps(&self) -> VolumeCaps;                 // sec 6
...
inversion of control, itemized in sec 9.

### The four contract rules

1. **Batched, not per-file.** `push_batch`, never `push`. STANDARDS sec 4.3 warns
...
into per-parent `Vec`s guarded by a linear `contains()` scan - O(k^2) per
...
| **Sink contention** | Adding threads makes it 15% faster, not 5x | Thread-local batches of a few thousand; dumb sink |
...
Following STANDARDS sec 1 - one concept per file, no `utils.rs`.
...
+-- parser/
|   +-- mod.rs          FsScanner trait + ScannerRegistry (STANDARDS sec 4.3)
...
New `Error` variants belong in `error.rs` per STANDARDS sec 4.1 - `Volume`,
`Filesystem`, `Unsupported` - each carrying the volume or offset that failed.
`Error::Cancelled` already exists.

**Logging** is `tracing` (STANDARDS sec 4.2), and the hot path gets none of it. One
...
6. Add a fixture image under `emfit-core/tests/fixtures/` and an integration
   test asserting a known tree (STANDARDS sec 5.1: every new format gets a parser
...
| ZFS | very hard | Compression, RAIDZ reconstruction, dedup. Stresses the model itself (sec 6). |
...
- **Where the scan file format lands** (features.md sec 1.6). It is close to a
