...
3. **Bulk sequential read.** `[v1-partial]` Read multi-megabyte chunks along the extent map, not record-at-a-time.
   - `[new]` **Clamp each read to the current extent.** v1 computes the offset of the first record in a batch and then reads `count x 1024` bytes straight through, so any batch spanning a fragment boundary silently ingests unrelated on-disk data. Reads must stop at the extent edge.
...
7. **Build the index** (sec 1.3) and **roll up sizes** (sec 1.4).
...
Shared by every filesystem; see [architecture.md](./architecture.md) sec 7 for the
...
- `[new]` **CSR child lists.** Count children per parent, prefix-sum, fill - one O(n) pass. v1 pushes into a per-parent `Vec` guarded by a linear `contains()` scan, which is O(k^2) per directory, and then repeats the entire operation a second time in `build()`. On directories with tens of thousands of entries this dominates the whole scan.
...
- `[new]` **BenchLog** per STANDARDS sec 5.5: append-only JSONL in `logs/`, one event per scan with `duration_ms`, record count, file count, volume, mode (physical/volume/fallback). Local only.
...
- `[v1]` **Keyboard shortcuts dialog** and **About** dialog. `[new]` The About panel must surface the bundled license texts (Inter OFL, Bootstrap Icons MIT) per STANDARDS sec 5.6 - the scaffold already has this component.
...
- `[new]` **Update checking.** EmFit looks itself up by name in the release site's shared manifest and compares the published version against its own. **Only when asked**: Help > Check for updates, or a startup check the user opts into in Settings > Updates, which stays silent unless there is a newer version and reports nothing if the site is unreachable. A fresh install talks to nothing. The dialog shows the release notes the manifest's `readme_file` points at, rendered as plain text because it is remote content. Installing replaces the running executable in place - EmFit ships as one portable executable, so an update is one file replacing one file: the running binary is renamed aside, the download is copied over it, and the app restarts into the new one, deleting the renamed original on the next start. The install directory is pre-flighted with a real write before anything moves, and the only irreversible step reverses itself if the copy fails, so a failed update never leaves the app without a working executable. When it cannot replace itself - a directory needing Administrator, read-only media, an executable path Windows will not report - nothing is touched, the download stays staged under the product data directory (`updates/`), and the user gets one sentence saying which of those it was and what to do (`service::update::apply::Blocker`), with the same reason in the log. The manifest publishes no checksum today, so `service::update::verify` is wired and inert: adding a `sha256` field per release turns integrity checking on with no code change.
...
- **Typed IPC contract** in `frontend/src/lib/ipc.ts` mirroring the Rust command signatures, with `npm run check` clean per STANDARDS sec 5.1.
...
- Remote telemetry of any kind (STANDARDS sec 5.5 - these tools handle forensic evidence).
