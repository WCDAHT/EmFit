...
/// Reverse-DNS qualifier (the `com` in `com.brunch.app-name`). Used only on
/// macOS, where paths are bundle-style; ignored on Windows and Linux.
pub const QUALIFIER: &str = "com";

/// Company / organization name. Becomes the first path segment under
/// `%APPDATA%` on Windows.
pub const COMPANY: &str = "Brunch";

/// Product name. Matches `productName` in `tauri.conf.json`.
pub const PRODUCT: &str = "app-name";
